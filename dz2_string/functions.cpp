...
void parse(const char* text){
	...
	while (tek){
		if(tek==' '){	
				...
				if(looks_like_number==1) {if (my_handler_for_ints !=nullptr ){
				...
				if(looks_like_number==0) {if (my_handler_for_strings !=nullptr ){my_handler_for_strings(tek_token.c_str()); }}
			

				tek_token.clear(); 


				j=i; while( (j<=(my_len-1)) &&(text[j]==' ')){j++;}

				i=j-1;

				if ((i<(my_len-1)) && (text[i+1] >= '0' && text[i+1] <= '9')) {looks_like_number=1;}
