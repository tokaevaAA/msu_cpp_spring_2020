...
void
my_func_for_strings (const char *tek_word)
{
  cnt_strings += 1;
...
main ()
{
  ...
  if ((cnt_ints != 0) || (cnt_strings != 0))
    {
      printf
	("Error: no my_functions registered, but cnt_ints != 0 or cnt_strings !=0!\n");
      return 1;
    }
  else
    {
  ...
  if ((cnt_ints != 4) || (cnt_strings != 0))
    {
      printf ("Error:  cnt_ints != 4 or cnt_strings !=0 !\n");
      return 2;
    }
  else
    {
  ...
  parse ("456");
  if (has_my_handler_for_end_work != 1)
    {
      printf ("Error: has_my_handler_for_end_work!=1\n");
      return 4;
    }
  else
    {
  ...
  printf ("cnt_ints=%d, cnt_strings=%d\n", cnt_ints, cnt_strings);
  if ((cnt_ints != 1) || (cnt_strings != 3))
    {
      printf ("Error: cnt_ints != 1 or cnt_strings !=3 !\n");
      return 5;
    }
  else
    {
